...
def run_lotka_volterra():
    ...
    data = sim.run()
    plot_results(data, "Lotka–Volterra Model")
...
def run_logistic_hollingII():
    ...
    data = sim.run()
    plot_results(data, "Logistic + Holling II Model")

def plot_results(data, title):
    plt.plot(data["time"], data["prey"], label="Prey", linewidth=2)
    plt.plot(data["time"], data["predator"], label="Predator", linewidth=2)
    plt.xlabel("Time")
